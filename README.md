...
![Stack](https://img.shields.io/badge/React_18-TypeScript-blue) ![Stack](https://img.shields.io/badge/Vite_6-Tailwind_CSS_3-purple) ![Supabase](https://img.shields.io/badge/Supabase-Self--Hosted-darkgreen) ![Caddy](https://img.shields.io/badge/Caddy-Reverse_Proxy-blue)
...
Checks prerequisites, clones the repo, installs dependencies, boots Docker + Supabase, and opens the dashboard. Zero prompts.
...
```powershell
git clone https://github.com/GGCryptoh/jarvis_inc.git; cd jarvis_inc; npm install; npm run jarvis
```
...
│  └── Supabase client (Postgres + Realtime)              │
├─────────────────────────────────────────────────────────┤
...
> **Or use the one-liner:** `curl -fsSL https://raw.githubusercontent.com/GGCryptoh/jarvis_inc/main/install.sh | bash`
...
Data persists across browser sessions in Postgres via Supabase.
...
| **Organizational Memory** | Memory extraction from conversations, founder profile, CEO prompt injection |
| **CEO Scheduler** | Visibility-aware interval, heartbeat, stuck task detection |
| **Decision Engine** | Rule-based evaluation, scheduled missions, proactive chat |
| **Supabase Realtime** | 6 table subscriptions via WebSocket |
| **Skill Execution** | Task dispatch, browser-side executor, multi-task missions |
| **Collateral** | Artifact browser with markdown rendering, export (PDF/MD), filtering |
| **CEO Management Actions** | 11 action types: hire/fire agents, create/schedule missions, budget requests |
| **Agent Chat** | Direct conversation with individual agents, agent-specific LLM calls |
| **Recurring Missions** | Cron-based scheduling with auto-dispatch |
| **Mission Detail Page** | Full verification UI: approve/archive/rerun, stats cards, task output |
| **Cost Tracking** | Per-call LLM usage logging, agent cost hover cards, financials from real data |

### Planned

| Feature | Description |
|---------|-------------|
| **CEO Skill Factory** | CEO creates skills from chat instead of dumping code inline |
| **Collateral Code Execution** | Syntax highlighting, Python/Pyodide runner, HTML preview |
| **Agent Communication** | Internal agent-to-agent coordination via CEO |
| **Governance** | Permissions, budget enforcement, kill switch |
...
| Missions | `/missions` | **Live** — Kanban + detail pages + approve/rerun |
...
| Audit | `/audit` | **Live** — real event log + export |
| Financials | `/financials` | **Live** — editable budget |
| Collateral | `/collateral` | **Live** — artifact browser + export |
| Settings | `/settings` | **Live** — system info + memory management |
...
| Icons | Lucide React |
| Database | Supabase self-hosted (Postgres 15 + PostgREST + Realtime) |
| Auth | Supabase GoTrue (email + passkeys/WebAuthn) |
| Realtime | Supabase Realtime (WebSocket) |
| Reverse Proxy | Caddy 2 (auto HTTPS) |
| LLM Streaming | Anthropic, OpenAI, Google, DeepSeek, xAI (5 providers) |
| Gateway | Express.js workspace gateway (Docker sidecar) |
...
├── package.json                  # React 18, Vite 6, Tailwind 3, Supabase
...
├── docker/
│   ├── docker-compose.yml        # Full stack: Supabase + Caddy + Gateway
│   ├── Caddyfile                 # Reverse proxy config (3 routes)
│   ├── .env.example              # All environment variables
│   ├── gateway/                  # Express.js skill execution gateway
│   └── supabase/
│       ├── kong.yml.template     # API gateway route template
│       └── migrations/           # Postgres schema
├── intelligence/                 # LLM prompt templates (14 default prompts)
├── marketplace/                  # Next.js marketplace hub (jarvisinc.app)
├── skills_repo/                  # Git submodule → github.com/GGCryptoh/jarvis_inc_skills
├── public/
│   ├── favicon.png
...
└── src/
    ...
    ├── lib/                      # Core logic (~30 modules)
    │   ├── database.ts           # Supabase-backed CRUD (~40 functions)
    │   ├── supabase.ts           # Supabase client singleton
    │   ├── skillExecutor.ts      # 8-tier skill execution engine
    │   ├── ceoScheduler.ts       # CEO autonomy scheduler
    │   ├── ceoDecisionEngine.ts  # Rule-based decision engine
    │   ├── managementActions.ts  # 11 CEO management action handlers
    │   ├── memory.ts             # Organizational memory + LLM extraction
    │   └── llm/                  # LLM providers (Anthropic, OpenAI, Google)
    ├── hooks/                    # React hooks (boot, scheduler, realtime)
    ...
    └── components/
        ...
        ├── Chat/                 # ChatView, ChatThread, OnboardingFlow
        ├── Approvals/            # Pending queue + history
        ├── Skills/               # Skill grid with execution + testing
        ├── Dashboard/            # Stats + ops table + fleet
        ├── Missions/             # Kanban board + detail pages
        ├── Vault/                # API key + OAuth management
        ├── Collateral/           # Task execution artifacts
        ├── Audit/                # Event log
        ├── Financials/           # Budget + LLM cost tracking
...
Skills are the easiest way to contribute. See the [skills repo](https://github.com/GGCryptoh/jarvis_inc_skills) for the schema and existing examples.

Join the community forum at [jarvisinc.app/forum](https://jarvisinc.app/forum).
