...
|   |-- README.md
|   |-- communication/
|   |   |-- read_email.json               # Read and parse incoming emails (Google OAuth)
|   |   |-- write_email.json              # Compose and send emails (Google OAuth)
|   |   |-- send_slack.json               # Post messages to Slack channels (Slack OAuth)
|   |   +-- schedule_meeting.json         # Create calendar events (Google OAuth)
|   |-- research/
|   |   |-- research_web.json             # Web search, page analysis, fact-checking (LLM)
|   |   |-- read_tweets.json              # Twitter/X search and profile analysis (LLM)
|   |   |-- research_reddit.json          # Reddit search and thread analysis (LLM)
|   |   |-- deep_search.json              # Multi-source deep research (LLM)
|   |   |-- browse_web.json               # Browser automation and data extraction (LLM)
|   |   +-- web_scraping.json             # CSS selector-based page scraping (LLM)
|   |-- creation/
|   |   |-- create_images.json            # AI image generation (OpenAI API key)
|   |   |-- generate_video.json           # AI video generation (OpenAI API key)
|   |   |-- write_document.json           # Long-form document drafting (LLM)
|   |   +-- generate_code.json            # Code generation, review, debugging (LLM)
|   +-- analysis/
|       |-- analyze_data.json             # Dataset analysis and visualization (LLM)
|       |-- analyze_image.json            # Image description and OCR (LLM vision)
|       |-- summarize_document.json       # Document summarization and outlining (LLM)
|       +-- translate_text.json           # Multi-language translation (LLM)
