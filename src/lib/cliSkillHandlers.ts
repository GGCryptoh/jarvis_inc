...
type CLIHandler = (commandName: string, params: Record<string, unknown>, apiConfig?: ApiConfig) => Promise<CLISkillResult>;

// ---------------------------------------------------------------------------
// Weather CLI handler (wttr.in)
// ---------------------------------------------------------------------------

// DEPRECATED: This handler will be replaced by declarative execution blocks in skill.json.
// See skills_repo/Official/ for the new directory-based skill packages.
async function executeWeatherCli(
  ...
  const baseUrl = apiConfig?.base_url ?? 'https://wttr.in';

  try {
    if (commandName === 'get_current') {
      const resp = await fetch(`${baseUrl}/${encodeURIComponent(location)}?format=j1`);
...
async function executeWhoisLookup(
  ...
  try {
    ...
    const resp = await fetch(`${baseUrl}/domain/${encodeURIComponent(domain)}`);
    if (!resp.ok) throw new Error(`RDAP ${resp.status}: ${resp.statusText}`);
    const data = await resp.json();
...
async function executeDnsLookup(
  ...
  const baseUrl = apiConfig?.base_url ?? 'https://cloudflare-dns.com/dns-query';
  const customHeaders = apiConfig?.headers ?? { Accept: 'application/dns-json' };
...
// DEPRECATED: These hardcoded handlers are being replaced by declarative request/response
// blocks in skill.json. New skills should use declarative execution or handler_file.
...
export async function executeCLISkill(
  ...
  return handler(commandName, params, apiConfig);
}
