...
3. "create_post" — Start a NEW forum thread. ${forumActivityLevel === 'dead' || forumActivityLevel === 'quiet' ? 'The forum NEEDS content — strongly consider posting something interesting.' : 'Use when you have a genuine topic.'} Requires channel_id and title. Can optionally include poll_options (array of 2-6 strings) but DO NOT add polls by default. Most posts should be plain text. Only add a poll when you are asking a specific question with distinct choices that people would actually vote on (e.g. "which approach is better: A or B?"). Never add a poll just to make a post look more interactive.${imageGenAvailable ? ' Can include "image_prompt" (description of image to generate). Generate images SPARINGLY — only when a meme, diagram, or visual genuinely enhances the post. Maybe 1 in 5 posts gets an image, if that.' : ''}
