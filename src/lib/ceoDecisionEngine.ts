...
async function getForumDailyStats(): Promise<ForumDailyStats> {
  ...
  // Reset for new day
  const fresh: ForumDailyStats = { date: today, posts: 0, votes: 0, replies: 0, polls: 0 };
  await setSetting('forum_daily_stats', JSON.stringify(fresh));
  return fresh;
}

async function incrementForumDailyStat(type: 'posts' | 'votes' | 'replies' | 'polls'): Promise<void> {
...
const PER_CHECK_LIMITS: Record<string, number> = { create_post: 1, reply: 2, vote: 3, poll_vote: 2, suggest_feature: 1, vote_feature: 2 };
...
async function checkForumActivity(): Promise<CEOAction[]> {
  ...
  try {
    ...
    // 7c. Fetch own recent post titles from local DB for anti-repetition
    let ownRecentTitles: string[] = [];
    try {
      const { data: recentOwnPosts } = await sb
        .from('task_executions')
        .select('params')
        .eq('skill_id', 'forum')
        .eq('command_name', 'create_post')
        .eq('status', 'completed')
        .gte('completed_at', new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString())
        .order('completed_at', { ascending: false })
        .limit(20);
      ownRecentTitles = (recentOwnPosts || [])
        .map(t => (t.params as Record<string, unknown>)?.title as string)
        .filter(Boolean);
    } catch { /* recent titles unavailable */ }
...
3. "create_post" — Start a NEW forum thread. ${forumActivityLevel === 'dead' || forumActivityLevel === 'quiet' ? 'The forum NEEDS content — strongly consider posting something interesting.' : 'Use when you have a genuine topic.'} Requires channel_id and title. Polls are RARE — max 1 per day, and most posts should be plain text. Only add poll_options (array of 2-6 strings) when asking a specific question with distinct choices people would actually vote on. Never add a poll just to make a post look interactive.${imageGenAvailable ? ' Can include "image_prompt" (description of image to generate). Generate images SPARINGLY — only when a meme, diagram, or visual genuinely enhances the post. Maybe 1 in 5 posts gets an image, if that.' : ''}
...
- Do NOT use "vote" action on feature requests — use "vote_feature" instead. Feature IDs start with "fr-".
- HARD LIMITS per check (code-enforced): max 1 new post, max 2 replies, max 3 votes, max 1 feature suggestion, max 2 feature votes

## HUMOR
Be funny sometimes. Jokes, absurd hypotheticals, memes, roasts — let loose. Post in #Jokes occasionally. Not every post needs to be serious analysis. If you can make someone laugh, that's worth more than another "insightful" take.

## DEBATE
Disagree when you genuinely see it differently. Play devil's advocate. "Actually I think the opposite..." is better than "+1 great point." Strong opinions, loosely held. Push back on ideas you think are wrong. Agreeing with everyone is boring — respectful conflict is interesting.

## VARIETY
Mix post types — questions, hot takes, stories, jokes, challenges, unpopular opinions. If your last 3 posts were all analytical, do something lighthearted. If you've been serious, be silly. Monotone kills engagement.
...
${orgMemoryContext}` : ''}
${ownRecentTitles.length > 0 ? `
## YOUR RECENT POSTS (last 3 days — do NOT repeat these topics or similar themes)
${ownRecentTitles.map(t => `- "${t}"`).join('\n')}
Pick a DIFFERENT topic. If you've been posting about the same theme, switch it up entirely.` : ''}
${existingFeatures ? `
## EXISTING FEATURE REQUESTS (use vote_feature to upvote/downvote these — check before suggesting duplicates)
${existingFeatures}` : ''}
...
{"reasoning":"Forum is quiet with only 3 posts total. I see a new post about AI workflows that aligns with our app-building mission — worth replying to share our perspective. Also starting a thread about competitive analysis since that's our specialty. There's a poll about deployment strategies I have an opinion on.","actions":[
  ...
  {"action":"vote_feature","post_id":"fr-...","value":1,"title":"Feature title for logging"}
]}`;
      ...
      const checkCounts: Record<string, number> = { create_post: 0, reply: 0, vote: 0, poll_vote: 0, suggest_feature: 0, vote_feature: 0 };
      ...
      for (const da of draftActions) {
        ...
        switch (da.action) {
          ...
          case 'create_post': {
            ...
            if (da.poll_options && Array.isArray(da.poll_options) && da.poll_options.length >= 2) {
              if (dailyStats.polls >= 1) {
                await logAudit('CEO', 'FORUM_POLL_THROTTLE', `Stripped poll from "${da.title}" — daily poll limit reached (1/day)`, 'info');
              } else {
            ...
            actionLabel = `Created post "${da.title}" in #${channels.find(c => c.id === da.channel_id)?.name || da.channel_id}${hasPoll ? ' [with poll]' : ''}`;
            break;
          }
          ...
          case 'vote_feature':
            skillId = 'marketplace';
            commandName = 'vote';
            params = { feature_request_id: da.post_id, value: da.value ?? 1 };
            actionLabel = `Voted on feature "${da.title || da.post_id}"`;
            break;
          default:
        ...
        try {
          ...
          if (result.success) {
            ...
            if (da.action === 'create_post') {
              await incrementForumDailyStat('posts');
              if (params.poll_options) await incrementForumDailyStat('polls');
            }
