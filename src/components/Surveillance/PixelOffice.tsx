...
interface PixelOfficeProps {
  ...
  /** Speech bubble text + position (CEO comms) */
  speechBubble?: { text: string; position: { x: number; y: number }; visible: boolean } | null;
  /** Floor upgrade ceremony state */
  upgradeFlash?: boolean;
  upgradeConfetti?: boolean;
...
export default function PixelOffice({
  ...
  return (
    <div
      className={`relative w-full h-full overflow-hidden ${floorPlannerActive ? 'cursor-crosshair' : ''} ${upgradeFlash ? 'animate-[screen-shake_0.4s_ease-out]' : ''}`}
    ...
    >
      ...
      {speechBubble && (
        <SpeechBubble
          ...
          position={speechBubble.position}
      ...
      {/* ---- Pixel Confetti ---- */}
      <PixelConfetti active={upgradeConfetti} onComplete={onConfettiComplete} />

      {/* ---- Level Up Banner ---- */}
      {upgradeLevel != null && (
        <LevelUpBanner level={upgradeLevel} active={upgradeLevel != null} />
      )}
    </div>
  );
}
