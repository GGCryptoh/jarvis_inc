...
import { getMonthlyUsage, getCurrentMonthSpend, getDailyUsage } from '../../lib/llmUsage'
...
export default function FinancialsView() {
  ...
  const [dailyData, setDailyData] = useState<{ day: number; date: string; llmCost: number; channelCost: number }[]>([]);
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const [budgetSaved, monthly, daily, spend] = await Promise.all([
  ...
  return (
    <div className="min-h-screen bg-jarvis-bg p-6">
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        {/* Daily Run Rate (Left) */}
        <div className="bg-jarvis-surface border border-white/[0.06] rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-jarvis-muted">Target</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-4 h-3 rounded-sm bg-cyan-500/80" />
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-56 text-jarvis-muted">
              ...
              <p className="text-xs mt-1 opacity-60">Spend will appear per-day as agents run tasks</p>
            </div>
          ) : (() => {
            const dailyTarget = monthlyBudget / daysInMonth;
            const dailyChartData = dailyData.map(d => ({ ...d, total: d.llmCost + d.channelCost }));
            const dailyMax = Math.max(dailyTarget, ...dailyChartData.map(d => d.total)) * 1.15;
            const targetPct = dailyMax > 0 ? (dailyTarget / dailyMax) * 100 : 0;

            return (
              <div className="relative h-56 px-1">
                {/* Target line */}
                <div
                  className="absolute left-0 right-0 border-t-2 border-dashed border-amber-400/50 z-10 pointer-events-none"
                  style={{ bottom: `${targetPct}%` }}
                >
                  <span className="absolute -top-4 right-0 text-[9px] font-mono text-amber-400/70">
                ...
                <div className="flex items-end gap-px h-full">
                  {dailyChartData.map(entry => {
                    const barPct = dailyMax > 0 ? (entry.total / dailyMax) * 100 : 0;
                    const isOver = entry.total > dailyTarget;
                    return (
                      <div key={entry.day} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                        <div
                          className={`w-full rounded-t-sm transition-all duration-300 min-h-[1px] ${isOver ? 'bg-red-500/70' : 'bg-cyan-500/70'}`}
                          style={{ height: `${barPct}%` }}
                        />
                        {/* Tooltip on hover */}
                        <div className="absolute bottom-full mb-1 hidden group-hover:block z-20">
                          <div className="bg-black/90 border border-zinc-700 rounded px-2 py-1 text-[9px] text-zinc-300 whitespace-nowrap font-mono">
                            {entry.date}: {formatCurrency(entry.total)}
                          </div>
                        </div>
                      </div>
                    );
          ...
          <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/[0.04]">
            <span className="text-[10px] text-jarvis-muted">
              Avg: <span className="text-jarvis-text font-mono">{formatCurrency(dailyRate)}/day</span>
            </span>
            <span className="text-[10px] text-jarvis-muted">
              Target: <span className="text-jarvis-text font-mono">{formatCurrency(monthlyBudget / daysInMonth)}/day</span>
            </span>
            <span className={`text-[10px] font-mono ${dailyRate > monthlyBudget / daysInMonth ? 'text-red-400' : 'text-emerald-400'}`}>
              {dailyRate > monthlyBudget / daysInMonth ? 'OVER PACE' : 'UNDER PACE'}
            </span>
          </div>
        </div>

        {/* Monthly Budget vs Actual (Right) */}
        <div className="bg-jarvis-surface border border-white/[0.06] rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-sm font-semibold text-jarvis-text uppercase tracking-wider">
