...
export default function ChatView() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Default to the most recent active conversation — but if it's stale
      // (>20 min since last activity), open a fresh chat instead
      const STALE_THRESHOLD_MS = 10 * 60 * 1000; // 10 minutes
      const active = convos.find(c => c.status === 'active');
      if (active) {
        const lastActivity = new Date(active.updated_at + (active.updated_at.endsWith('Z') ? '' : 'Z')).getTime();
        const isStale = Date.now() - lastActivity > STALE_THRESHOLD_MS;
        if (isStale) {
          // Auto-create a fresh chat
  ...
  const handleClearAll = useCallback(async () => {
    // Delete all non-archived, non-onboarding conversations
    const toDelete = conversations.filter(c => c.status !== 'archived' && c.type !== 'onboarding');
    ...
    // Auto-create a fresh chat
    await handleNewChat();
  }, [conversations, refreshConversations, handleNewChat]);
