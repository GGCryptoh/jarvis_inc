      ...
      <div className="bg-jarvis-surface border border-jarvis-border rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-zinc-200 leading-relaxed whitespace-pre-line">{mission}</p>
          </div>
          <button
            onClick={openEdit}
...
// ---------------------------------------------------------------------------
// System Health Card — live service status dots
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="bg-jarvis-surface border border-jarvis-border rounded-lg p-4 flex flex-col">
      <div className="flex items-center gap-2 mb-3">
        <span className={`w-2.5 h-2.5 rounded-full ${allUp ? 'bg-emerald-400' : 'bg-red-400'}`} />
        <span className="text-xs font-medium text-jarvis-muted uppercase tracking-wider">
          System Health
        </span>
      </div>
...
export default function DashboardView() {
  ...
  const { services, allUp } = useSystemHealth()
  ...
  useEffect(() => {
    const refresh = () => {
      ...
      getCurrentMonthSpend().then(s => setTotalSpend(s.total))
    }
    refresh()
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      {/* Primary Mission + System Health */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
        <div className="lg:col-span-3">
          <MissionCard />
        </div>
        <div className="lg:col-span-2">
          <SystemHealthCard services={services} allUp={allUp} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <StatCard
          label="System Status"
          value={allUp ? 'Online' : 'Degraded'}
          sublabel={allUp ? 'All services healthy' : `${services.filter(s => s.status === 'down').length} service(s) down`}
        />
      </div>
