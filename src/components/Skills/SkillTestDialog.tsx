...
export default function SkillTestDialog({ skill, open, onClose }: SkillTestDialogProps) {
  ...
  const connectionType = useMemo(() => {
    ...
    return typeof conn === 'string' ? conn : (conn as Record<string, unknown>)?.type as string ?? '';
  }, [skill.connection]);
  const isCLI = connectionType === 'cli';
  const isNonLLM = connectionType === 'cli' || connectionType === 'api_key' || connectionType === 'none';
            ...
            <div className="flex items-center gap-1.5 font-pixel text-[7px] tracking-wider text-zinc-600">
              {isNonLLM ? (
                <>
                  ...
                  <span className="text-cyan-500">{connectionType === 'cli' ? 'HTTP / CLI' : 'API'}</span>
                  {skill.apiConfig?.api_model ? (
                    <span className="text-zinc-400 ml-1">— {skill.apiConfig.api_model}</span>
                  ) : (
