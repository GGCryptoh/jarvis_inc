...
async function handleImageGenerate(
  ...
  if (supabase && imageUrl) {
    try {
      ...
      if (imgResp.ok) {
        ...
        const storagePath = `edge/${Date.now()}-dalle3.png`;
  ...
  // Build markdown output for Collateral rendering
  const outputMd = [
    `## Generated Image`,
    ``,
    ...
    ``,
    `**Prompt:** ${prompt}`,
    revisedPrompt !== prompt ? `**Revised prompt:** ${revisedPrompt}` : "",
    `**Size:** ${size} | **Quality:** ${quality} | **Style:** ${style}`,
  ].filter(Boolean).join("\n");

  const summaryText = `Generated image: ${revisedPrompt}`;
...
async function executeApiKeySkill(
  ...
  if (!handler) {
    ...
    if (fallbackKey) {
      return API_KEY_HANDLERS[fallbackKey](vaultEntry.key_value, model, definition, params, supabase);
    }
    throw new Error(`No handler for api_key skill "${skillId}" command "${commandName}". Supported: ${Object.keys(API_KEY_HANDLERS).join(", ")}`);
  }

  return handler(vaultEntry.key_value, model, definition, params, supabase);
}
